...
def parse_args():
    ...
    p.add_argument(
        "--cutmix-alpha", type=float, default=0.0,
        help="CutMix Beta distribution parameter (0 = off). Typical value: 1.0.",
    )
    p.add_argument(
    ...
    if args.mixup_alpha > 0 and args.cutmix_alpha > 0:
        p.error("--mixup-alpha and --cutmix-alpha are both batch-mixing methods; enable only one.")
...
def train_one_epoch_standard(model, loader, criterion, optimizer, device):
    """Standard training loop (no batch mixing)."""
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
...
def train_one_epoch_cutmix(model, loader, criterion, optimizer, device, alpha):
    """Training loop with CutMix data augmentation.
...
def checkpoint_name_for(args):
    """Build a descriptive checkpoint file name for the active regularizers."""
    parts = ["resnet18", args.dataset]
    if args.dropout > 0:
...
def main():
    ...
    if args.mixup_alpha > 0:
        reg_flags.append(f"Mixup(alpha={args.mixup_alpha})")
    if args.cutmix_alpha > 0:
        reg_flags.append(f"CutMix(alpha={args.cutmix_alpha})")
    if args.cutout_length > 0:
    ...
    checkpoint_name = checkpoint_name_for(args)
    best_val_acc = 0.0
    ...
    print(f"Best checkpoint: {os.path.join(args.checkpoint_dir, checkpoint_name)}")
