...
| **CutMix** | `--cutmix-alpha` | `train_cutmix.py` | [Yun et al., 2019](https://arxiv.org/abs/1905.04899) |
| **Cutout** | `--cutout-length` | `train_cutout.py` | [DeVries & Taylor, 2017](https://arxiv.org/abs/1708.04552) |

All methods can also be combined via the unified script `train.py`, except
Mixup and CutMix, which are mutually exclusive batch-mixing methods.
...
python train_cutmix.py --plot
python train_cutmix.py --dataset cifar100 --plot
...
# CutMix + dropout on CIFAR-10
python train.py --dropout 0.3 --cutmix-alpha 1.0 --plot

# Combine dropout + label smoothing + Mixup + Cutout on CIFAR-100
python train.py --dataset cifar100 --dropout 0.3 --label-smoothing 0.1 --mixup-alpha 0.2 --cutout-length 16 --plot
```

`--mixup-alpha` and `--cutmix-alpha` cannot both be greater than 0 in the
same run. Enable one batch-mixing method at a time.
...
├── train_cutmix.py            # CutMix data augmentation
├── train_cutout.py            # Cutout data augmentation
├── utils.py                   # Data loaders, Cutout transform, Mixup/CutMix helpers, visualization
├── requirements.txt
├── .gitignore
└── README.md
```
...
| `--cutmix-alpha` | 0.0 | CutMix Beta distribution parameter (0 = off) |
...
### CutMix
Creates virtual training examples by replacing a random rectangular region of
one image with the same region from another image in the batch. Labels are
mixed according to the replaced area, which encourages the model to use less
localized evidence and improves robustness. Controlled via `--cutmix-alpha`
(typical value: 1.0). In the unified script, CutMix and Mixup cannot be enabled
at the same time.
