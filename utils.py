...
def rand_bbox(size, lam):
    """Return a random CutMix bounding box for an NCHW tensor shape."""
    _, _, h, w = size
...
def cutmix_data(x, y, alpha=1.0):
    ...
    Classifiers with Localizable Features", 2019.  https://arxiv.org/abs/1905.04899

    Returns mixed inputs, pairs of targets, and the area-adjusted mixing
    coefficient lambda.
    """
    if alpha > 0:
        lam = np.random.beta(alpha, alpha)
...
def cutmix_criterion(criterion, pred, y_a, y_b, lam):
    """Compute CutMix loss as an area-weighted combination."""
    return lam * criterion(pred, y_a) + (1 - lam) * criterion(pred, y_b)
